"""
Memory Forest 메인 AI 처리 DAG - 기존 AI 서비스와 repository 코드 호환
"""
...
from config import DAG_DEFAULT_ARGS, BATCH_SIZES, SCHEDULES, DEFAULT_ARGS, LOCAL_TZ
    ...
    for game in pending_games:
        try:
            ...
            if ai_response.get("status") == "success":
                # AI 분석 성공 - 기존 repository 형식에 맞춰 저장
                ai_result = ai_response["result"]
                
                # 완료 상태로 업데이트 (기존 repository save_ai_analysis_result 호환)
                try:
...
dag = DAG(
    ...
    description='Memory Forest AI 게임 분석 메인 워크플로우 - 기존 코드 호환',
    schedule_interval=SCHEDULES['main_processing'],  # 30분마다 실행
    catchup=False,
    max_active_runs=1,
