...
import java.time.LocalDateTime;
 ...
 public class AuthService implements UserDetailsService {
     ...
     public boolean updatePassword(String loginId, String newPassword) {
         try {
             ...
             if (userOptional.isEmpty()) {
                 log.error("비밀번호 업데이트 실패: 사용자 없음 - {}", loginId);
                 return false;
             }
             ...
             log.info("비밀번호 업데이트 성공: {}", loginId);
             return true;

         } catch (Exception e) {
             log.error("비밀번호 업데이트 중 오류 발생: {}", loginId, e);
             return false;
         }
     }
     ...
     public Optional<User> findByLoginIdAndEmail(String loginId, String email) {
         return userService.findByLoginIdAndEmail(loginId, email);
     }
 }